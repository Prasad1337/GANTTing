...
import net.sourceforge.ganttproject.action.CalculateAutoLevel;
...
class GanttChartTabContentPanel extends ChartTabContentPanel implements GPView {
  ...
  private final CalculateAutoLevel myAutoLevel;
  ...
  GanttChartTabContentPanel(IGanttProject project, UIFacade workbenchFacade, TaskTreeUIFacade treeFacade,
    ...
    myAutoLevel = new CalculateAutoLevel(project.getTaskManager(), uiConfiguration, workbenchFacade);
    addChartPanel(createSchedulePanel());
  }

  private Component createSchedulePanel() {
    return new ToolbarBuilder().withBackground(
        myWorkbenchFacade.getGanttChart().getStyle().getSpanningHeaderBackgroundColor()).addButton(myCriticalPathAction).addButton(myAutoLevel).addButton(
                myBaselineAction).build();
  }
