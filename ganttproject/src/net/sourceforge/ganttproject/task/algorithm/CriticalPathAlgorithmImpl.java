...
public class CriticalPathAlgorithmImpl implements CriticalPathAlgorithm {
  ...
  private ArrayList<Task> tempTasks = new ArrayList<Task>();

  @Override
  public ArrayList<Task> getCriticalTimes()
  {
  ...
  public Task[] getCriticalTasks() {
    ...
    for(Node curNode: task_node.values())
    {
    	p.calculateLatestDates(curNode);
    	Task t = curNode.task;
    	long[] arr = {curNode.est.getTime(), curNode.eft.getTime(), curNode.lst.getTime(), curNode.lft.getTime()};
    	t.setCriticalTimes(arr);
    	this.tempTasks.remove(t);
    	this.tempTasks.add(t);
    }
    //
    result.addAll(p.run());
