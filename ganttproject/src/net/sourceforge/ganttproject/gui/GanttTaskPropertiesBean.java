...
public class GanttTaskPropertiesBean extends JPanel {
  ...
  private Task.Role originalRole;
  ...
  private void constructGeneralPanel() {
    ...
    propertiesPanel.add(new JLabel(language.getText("tableColResourceRole")));
    ...
    for (Task.Role r : Task.Role.values()) {
        roleComboBox.addItem(language.getText(r.getI18rKey()));
      }
    roleComboBox.setEditable(false);
    propertiesPanel.add(roleComboBox);
...
//    });
  ...
  public void applySettings() {
    for (int i = 0; i < selectedTasks.length; i++) {
      ...
      if(this.originalRole != getRole()) {
    	  mutator.setRole(getRole());
      }
  ...
  private void setSelectedTaskProperties() {
    ...
    roleComboBox.setSelectedIndex(originalRole.ordinal());
  ...
  private Task.Role getRole() {
	    return Task.Role.getRole(roleComboBox.getSelectedIndex());
  }
